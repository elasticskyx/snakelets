...
import cgi,os,sys,types,glob,time,re,pickle
import email.Utils       
...
class ArticleStats(Snakelet):
    ...
    def init(self):
        ...
        ctx.registeredusers=set()  # a set of user ids
        ctx.registeredbloggerusers=set()  # a set of user ids
        ...
        ctx.Scheduler.add_interval_task(self.scheduled_UpdateUserListAndArticleCount, "ArticleCount update", start, interval, ctx.Scheduler.method.sequential, [], None)
        start+=2; interval=self.getWebApp().getConfigItem("blogentrydate_updateperiod")
        ctx.Scheduler.add_interval_task(self.scheduled_UpdateBlogEntryDates, "BlogEntry dates scanner", start, interval, ctx.Scheduler.method.sequential, [], None)
        start+=2; interval=self.getWebApp().getConfigItem("articlesinmonth_updateperiod")
        ctx.Scheduler.add_interval_task(self.scheduled_UpdateArticlesInMonth, "Articles in month scanner", start, interval, ctx.Scheduler.method.sequential, [], None)
        start+=2; interval=self.getWebApp().getConfigItem("activearticles_updateperiod")
        ctx.Scheduler.add_interval_task(self.scheduled_UpdateActiveArticles, "Active articles scanner", start, interval, ctx.Scheduler.method.sequential, [], None)
    ...
    def scheduled_UpdateArticlesInMonth(self):
        log.debug("(scheduled) updating articles in month")
    ...
    def scheduled_UpdateUserListAndArticleCount(self):
        log.debug("(scheduled) updating article counts")
        counts={}
        users=self.getAppContext().storageEngine.listUsers(False)   # also update the user list
        for userid in users:
    ...
    def scheduled_UpdateBlogEntryDates(self):
        log.debug("(scheduled) updating blog entry dates")
        dates={}
        users=self.getAppContext().storageEngine.listUsers(False)   # also update the user list
        for userid in users:
    ...
    def scheduled_UpdateActiveArticles(self):
        log.debug("(scheduled) updating active articles")
        users=self.getAppContext().storageEngine.listUsers(False)   # also update the user list
        active={}
        for userid in users:
    ...
    def user_UpdateActiveArticles(self, userid, activeArticles=None):
        ...
        sortlist=[]
        leregex=re.compile("<lastedited>(\d+)</lastedited>")
        dtregex=re.compile('<timestamp\s+date="([0-9-]+)"\s+time="([0-9:]+)"')
...
#
#   This snakelet serves the captcha images that is on the current session.
#
class Captcha(Snakelet):

    def requiresSession(self):
        return self.SESSION_REQUIRED

    def getDescription(self):
        return "serves the current captcha image"

    def serve(self, request, response):
        captcha=request.getSessionContext().currentCaptcha
        io=StringIO.StringIO()
        captcha.writeImage(io)
        response.setEncoding(None)  # clear any custom output encoding
        response.setContentLength(io.tell())
        response.setContentType("image/jpeg")
        response.getOutput().write(io.getvalue())
