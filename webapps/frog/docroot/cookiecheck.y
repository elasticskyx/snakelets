...
<%@pagetemplatearg=title=Cookies%>
<%@pagetemplatearg=left=Cookie problem%>
<%@session=no%>
<%

# This method uses redirection to itself to check if the session cookie is accepted by the browser.
...
<p>&nbsp;</p>
<p><span class="count"><strong>Why cookies?</strong> The cookie is used to store your user identification,
