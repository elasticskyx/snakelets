...
from frog.xmlstorage.ids import obtainIDGenerator
...
class XMLStorageEngine:
    ...
    def __init__(self, webapp, userid=None):
        if webapp:
            ...
            self.webapp.getContext().lockedBlogs=set()
        ...
        if self.userid:
            ...
            self.idGenerator = obtainIDGenerator(self.storagepath)  # NOTE: gets a singleton instance IDGenerator for the given path
    ...
    def getNumberOfComments(self, date, Id):
        ...
        # XXX this implementation is slow; it scans the comments XML file to extract the commentlist count...
        filename = Comments.determineFileName(self.storagepath, date, Id)
        if os.path.isfile(filename):
            regex=re.compile(r'<commentlist\s+count="([0-9]+)"')
    ...
    def _loadXML(clazz, storageengine, rootnode):
        ...
        for child in rootnode.childNodes:
            if child.nodeType==xml.dom.Node.ELEMENT_NODE:
                ...
                elif child.nodeName=="usecaptcha":
                    obj.usecaptcha = child.firstChild.nodeValue.lower() == "true"
