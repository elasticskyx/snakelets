...
idgenerators={}   # path-->_IDGenerator object
def obtainIDGenerator(path):
    global idgenerators, factorylock
    factorylock.acquire()
    try:
        try:
            gen=idgenerators[path]
            return gen
        except KeyError:
            idgenerators[path]=_IDGenerator(path)
            return idgenerators[path]
    finally:
        factorylock.release()
...
if __name__=="__main__":
    import tempfile
    gen=obtainIDGenerator(tempfile.gettempdir())
