...
class Submit:
    ...
    def prepareEditBlogEntry(self, fromSession=False):
        log.debug("Process prepareEditBlogEntry; requesturl="+self.Request.getRequestURL())
        if self.User:
        ...
        if hasattr(self.SessionCtx,"currentEntry") and (fromSession or self.Request.getParameter("action")):
            ...
            pId = self.Request.getParameter("id")
            if pId:
                Id=int(pId)
            else:
                self.abort("missing article id parameter")
            entry=None
        ...
        if not entry:
            try:
                ...
                log.error("exception occurred when trying to load article %d: %s" % (Id, x))
