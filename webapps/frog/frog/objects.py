...
#
class User(snakeserver.user.LoginUser, storageobjects.User):
    __slots__=['version','email','homepage','displaystrings','metakeywords','metadescription', 'categories','links',
               ...
               'mailnotify', 'usepuzzles', 'usecaptcha', 'countreads', 'rssenabled', 'customfrontpage', 'smileys', 'commentsmileys']
    ...
    def __init__(self, storageengine, userid, password, accounttype, passwordhash=None, directory=None):
        snakeserver.user.LoginUser.__init__(self, userid, password, "", passwordhash=passwordhash, privileges=None)
        ...
        self.colorstyle=0
        self.smileycolor=0
        self.mailnotify = self.customfrontpage = self.smileys = self.usecaptcha = False
...
class BlogEntry(storageobjects.BlogEntry):
    __slots__=['datetime', 'lastedited', 'numedits', 'category', 'articletype', 'title', 'text', 'text2',
               'allowcomments','smileys']
    def __init__(self, storageengine, category, title, text, text2=None, articletype="normal", allowcomments=True, smileys=False):
...
def test():
    ...
    se1=StorageEngine(None, (tempfile.gettempdir(), "irmen") )
    se1.createBlogDirsForToday()
